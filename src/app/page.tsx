...
export default function Home() {
  ...
  const typedDetailRef = React.useRef(null);
  const animateRef = React.useRef(null);
  var timeout: any = null;

  React.useEffect(() => {
    const typedTitle = new Typed(typedRef.current, {
      // strings: ['<i>Image</i> to animated svg.'],
      strings: [
        'ANIMATED YOUR PHOTO',
        'CONVERT YOUR PHOTO',
        'EXPORT YOUR PHOTO',
    ...
    const typedDetail = new Typed(typedDetailRef.current, {
      strings: [
        'TO SVG PATH.',
        'TO YOUR WEBSITE.',
        'TO GIF OR VIDEO.',
    ...
    const resize = (el: any, padding?: any) => {
    ...
    const spherePathEls = (animateRef.current as unknown as Document).querySelectorAll('.sphere path');
    const pathLength = spherePathEls?.length;
    var aimations: any[] = [];
  
    fitElementToParent(animateRef.current);
      ...
      begin: function() {
        for (var i = 0; i < pathLength; i++) {
          aimations.push(anime({
            ...
            stroke: {value: ['rgba(107, 33, 168, 0)', 'rgba(200, 60, 200, 0)'], duration: 1000},
            translateX: [2, -4],
            translateY: [2, -4],
            easing: 'easeOutQuad',
            autoplay: false
  ...
  return (
    <>
      <div className="-z-10 absolute w-screen h-screen flex items-center justify-center">
        <div className="animation-wrapper w-[800px] h-[800px]">
          <div className="sphere-animation" ref={animateRef}>
            <svg className="sphere" viewBox="0 0 440 440" fillOpacity="0.1" stroke="rgba(107, 33, 168, .5)">
              <defs>
                <linearGradient id="sphereGradient" x1="5%" x2="5%" y1="0%" y2="15%">
                  <stop stopColor="rgba(107, 33, 168, .5)" offset="0%"/>
                  <stop stopColor="rgba(200, 60, 200, .5)" offset="50%"/>
                  <stop stopColor="rgba(80, 20, 130, .5)" offset="100%"/>
                </linearGradient>
              </defs>
      ...
      <div className="flex flex-col w-screen h-screen bg-transparent">
        ...
        <div className="flex-grow flex flex-col items-center justify-center">
          <div className="h-[150px]">
            <h1 className="text-[100px] text-purple-800" ref={typedRef} />
          </div>
          <div className="h-16">
            <span className="text-4xl	text-purple-950 border-4 p-3" ref={typedDetailRef} />
          </div>
          <div className="mt-36 z-20 absolute w-screen h-screen bg-transparent flex justify-center items-end">
            <Button className="mb-40 bg-purple-950" onClick={scrollToGenerate}>
              <span className="font-bold text-xl">Convert Now</span>
            </Button>
          </div>
        </div>
      </div>
